...
class Session:
    ...
    pinned: bool = False  # explicit low-priority fallback set via `grit session pin`

    # ── Convenience ──────────────────────────────────────────────────────────

    def is_expired(self) -> bool:
        if self.locked or self.pinned:
            return False
    ...
    def ttl_remaining_seconds(self) -> int:
        """Seconds until expiry; 0 if already expired; -1 if locked or pinned."""
        if self.locked or self.pinned:
            return -1
        now = datetime.now(timezone.utc)
        expiry = datetime.fromisoformat(self.expires_at)
    ...
    def touch(self, ttl_seconds: int = DEFAULT_SESSION_TTL_SECONDS) -> None:
        ...
        if not self.locked and not self.pinned:
